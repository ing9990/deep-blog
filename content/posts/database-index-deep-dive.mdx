---
title: "Database Index (데이터베이스 인덱스)"
...
keywords:
  - 복합 인덱스
summary: "B+Tree 기반 인덱스의 내부 동작 원리를 이해하고, 복합 인덱스의 리프 노드 배치부터 등호 먼저·정렬 마지막 원칙, 인덱스 개수 결정까지 실전 설계 기준을 얻습니다."
category: database
draft: false
---
...
**높은 fanout**: 이진 트리는 노드당 자식이 2개뿐이라 트리 높이가 $\log_2 n$입니다. 2,100만 건이면 약 24레벨 — 한 건 찾는 데 24번의 Random I/O가 필요합니다. B+Tree는 노드 하나를 **디스크 페이지(보통 16KB)**에 맞춰 수백 개의 키를 담으므로 fanout이 수백입니다. $\log_{200} 21000000 \approx 3.2$ — 불과 3~4레벨이면 충분합니다.
...
![세컨더리 인덱스의 이중 탐색 — 인덱스 리프에서 PK를 꺼낸 뒤 클러스터드 인덱스를 재탐색하는 과정](/images/database-index-deep-dive-dual-lookup.svg)
...
<Callout type="warning" title="복합 인덱스 컬럼 순서 원칙">
  등호(`=`) 조건 컬럼을 선두에, 정렬(`ORDER BY`) 또는 범위(`BETWEEN`, `>=`) 컬럼을 마지막에 배치합니다. 등호 컬럼끼리의 카디널리티 차이는 중요하지 않습니다 — 어차피 `=` 조건은 정확히 하나의 파티션으로 점프하기 때문입니다.
</Callout>
