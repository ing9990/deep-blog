...
keywords:
  - Full Table Scan
  - 풀 테이블 스캔
summary: "Full Table Scan이 언제 문제이고 언제 정상인지 구분할 수 있습니다. 옵티마이저가 인덱스 대신 Full Table Scan을 선택하는 이유와 Sequential I/O vs Random I/O 트레이드오프를 이해합니다."
category: database
draft: false
---

Full Table Scan은 "느린 쿼리의 원인"으로만 알려져 있지만, 실제로는 옵티마이저가 **의도적으로 선택하는 경우**도 있습니다. 무조건 나쁜 것이 아니라, 언제 문제이고 언제 괜찮은지를 구분하는 것이 핵심입니다.

<Callout type="info" title="이 글의 학습 목표">
  1. Full Table Scan이 무엇이고 왜 발생하는지 이해한다
  2. Sequential I/O와 Random I/O의 차이를 구분할 수 있다
  3. Full Table Scan이 문제인 상황과 괜찮은 상황을 판단하는 기준을 얻는다
</Callout>
...
<Callout type="warning" title="Full Table Scan을 이해하는 핵심">
  Full Table Scan의 성능 특성은 **디스크 I/O 패턴**으로 결정됩니다. 순차적으로 읽느냐(Sequential), 여기저기 점프하며 읽느냐(Random)의 차이가 "같은 양을 읽어도 속도가 다른" 이유입니다.
</Callout>
...
![Sequential I/O vs Random I/O — 디스크 헤드 이동 패턴 비교](/images/full-table-scan-io-comparison.svg)
